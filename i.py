...
def save_table_info_to_file(table_info, file_path):
    with open(file_path, 'w') as file:
        file.write(f"Table Name: {table_info.name}\n")
        file.write("Columns:\n")
        for column in table_info.columns:
            file.write(f"\tName: {column.name}, Type: {column.data_type}, Not Null: {column.not_null}\n")
        ...
        file.write("Unique Constraints:\n")
        for constraint_name, columns in table_info.uniques.items():
            file.write(f"\t{constraint_name}: {', '.join(columns)}\n")
        file.write("Not Null Constraints: " + ', '.join(table_info.not_nulls) + "\n")
...
def main():
    ...
    try:
        ...
        # Save table information to files
        for idx, table in enumerate(tables, start=1):
            table_name = table[0]
            table_info = get_table_info(table_name, dbname, user, password, host, port)
            file_path = f"{idx}.txt"
            save_table_info_to_file(table_info, file_path)
            print(f"Table information saved to {file_path}")
